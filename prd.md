---
title: "Cliste — Product Requirements Document"
version: "1.0"
status: "Draft — Untuk Review Internal"
date: "31 Mei 2026"
language: "Bahasa Indonesia"
classification: "Rahasia Internal · Tidak untuk Disebarluaskan"
scope:
  - "App 1: Tender Platform"
  - "App 2: Internal Workspace"
phases:
  - "Phase 1 (Core)"
...
# CLISTE — Product Requirements Document
...
- **Klasifikasi:** Dokumen Rahasia Internal · Tidak untuk Disebarluaskan
...
1. [Pendahuluan](#1-pendahuluan)
   ...
   - 1.3 [Gambaran Ekosistem](#13-gambaran-ekosistem)
   - 1.4 [Target Pengguna](#14-target-pengguna)
2. [App 1 — Tender Platform](#2-app-1--tender-platform)
   - 2.1 [Gambaran Umum](#21-gambaran-umum)
   - 2.2 [Platform Tender yang Didukung](#22-platform-tender-yang-didukung)
   - **PHASE 1 · Core Tender Management**
     - 2.3 [Deskripsi Fitur Phase 1](#23-deskripsi-fitur-phase-1)
     - 2.4 [Kebutuhan Fungsional — App 1 Phase 1](#24-kebutuhan-fungsional--app-1-phase-1)
     - 2.5 [User Stories — App 1 Phase 1](#25-user-stories--app-1-phase-1)
     - 2.6 [Kebutuhan Non-Fungsional — App 1 Phase 1](#26-kebutuhan-non-fungsional--app-1-phase-1)
     - 2.7 [Kriteria Penerimaan — App 1 Phase 1](#27-kriteria-penerimaan--app-1-phase-1)
   - **PHASE 2 · Proposal AI Generator**
     - 2.8 [Deskripsi Fitur Phase 2](#28-deskripsi-fitur-phase-2)
     - 2.9 [Kebutuhan Fungsional — App 1 Phase 2](#29-kebutuhan-fungsional--app-1-phase-2)
     - 2.10 [Kriteria Penerimaan — App 1 Phase 2](#210-kriteria-penerimaan--app-1-phase-2)
3. [App 2 — Internal Workspace](#3-app-2--internal-workspace)
   - 3.1 [Gambaran Umum](#31-gambaran-umum)
   - 3.2 [Matriks Peran & Akses — App 2](#32-matriks-peran--akses--app-2)
   - **PHASE 1 · MODUL A · Project Management**
     - 3.3 [Deskripsi Fitur — Project Management](#33-deskripsi-fitur--project-management)
   - **PHASE 1 · MODUL B · Document Management**
     - 3.4 [Deskripsi Fitur — Document Management](#34-deskripsi-fitur--document-management)
   - **PHASE 1 · MODUL C · Client Portal**
     - 3.5 [Deskripsi Fitur — Client Portal](#35-deskripsi-fitur--client-portal)
   - 3.6 [Kebutuhan Fungsional — App 2 Phase 1](#36-kebutuhan-fungsional--app-2-phase-1)
   - 3.7 [User Stories — App 2 Phase 1](#37-user-stories--app-2-phase-1-pilihan-utama)
   - 3.8 [Kebutuhan Non-Fungsional — App 2 Phase 1](#38-kebutuhan-non-fungsional--app-2-phase-1)
   - 3.9 [Kriteria Penerimaan — App 2 Phase 1](#39-kriteria-penerimaan--app-2-phase-1)
   - **PHASE 2 · Fitur Lanjutan & Analytics**
     - 3.10 [Deskripsi Fitur Phase 2 — App 2](#310-deskripsi-fitur-phase-2--app-2)
     - 3.11 [Kebutuhan Fungsional — App 2 Phase 2](#311-kebutuhan-fungsional--app-2-phase-2)
   - **PHASE 3 · RAG Knowledge Base**
     - 3.12 [Gambaran Phase 3 — App 2](#312-gambaran-phase-3--app-2)
     - 3.13 [Deskripsi Fitur Phase 3 — App 2](#313-deskripsi-fitur-phase-3--app-2)
     - 3.14 [Kebutuhan Fungsional — App 2 Phase 3](#314-kebutuhan-fungsional--app-2-phase-3)
4. [Integrasi Antar Aplikasi](#4-integrasi-antar-aplikasi)
   - 4.1 [Integration Point: Tender Menang → Proyek Baru](#41-integration-point-tender-menang--proyek-baru)
   - 4.2 [Infrastruktur Bersama](#42-infrastruktur-bersama)
   - 4.3 [Autentikasi & Otorisasi](#43-autentikasi--otorisasi)
5. [Roadmap & Urutan Pengembangan](#5-roadmap--urutan-pengembangan)
   - 5.1 [Ringkasan Fase](#51-ringkasan-fase)
   - 5.2 [Urutan Pengembangan yang Direkomendasikan](#52-urutan-pengembangan-yang-direkomendasikan)
   - 5.3 [Dependensi Kritis](#53-dependensi-kritis)
   - 5.4 [Pertanyaan Terbuka](#54-pertanyaan-terbuka)
...
Cliste adalah perusahaan konsultan yang secara aktif mengikuti proses tender pengadaan dari berbagai instansi pemerintah maupun swasta, serta mengelola beberapa proyek aktif secara bersamaan. Saat ini, pencarian tender dilakukan secara manual melalui kunjungan rutin ke portal LPSE, CIVD, dan Geodipa. Pengelolaan proyek tersebar di berbagai platform yang tidak terintegrasi, dokumen proyek disimpan secara tidak terstruktur, dan komunikasi dengan klien masih bergantung pada WhatsApp dan email.

Kondisi ini mengakibatkan peluang tender yang terlewat, beban kerja yang tidak termonitor, dokumen yang sulit ditemukan kembali, serta tidak adanya transparansi terstruktur antara tim Cliste dan klien. Dokumen ini mendefinisikan kebutuhan produk untuk dua aplikasi terintegrasi yang akan membangun ekosistem digital internal Cliste.
...
Tender Platform adalah aplikasi yang mengotomatiskan seluruh proses pengelolaan tender di Cliste, mulai dari penemuan peluang hingga pengiriman proposal. Sistem ini bekerja secara proaktif di latar belakang sehingga tim hanya perlu fokus pada keputusan strategis, bukan pada rutinitas pemantauan portal pengadaan.
...
Sistem secara otomatis mengumpulkan data tender dari portal-portal yang dikonfigurasi menggunakan Playwright untuk merender halaman berbasis JavaScript. Proses berjalan terjadwal tanpa intervensi manual.
...
Setelah data terkumpul, sistem menganalisis kesesuaian antara persyaratan tender dengan profil, KBLI, dan kapabilitas Cliste. Hasilnya adalah skor prioritas yang membantu tim menentukan tender mana yang layak dikejar.
...
Antarmuka untuk tim mengelola status keputusan setiap tender secara terpusat, menggantikan pemantauan yang sebelumnya tersebar di spreadsheet dan percakapan WhatsApp.

- **Papan Kanban** dengan kolom status: `Ditemukan → Ditinjau → Dikejar → Diserahkan → Menang / Kalah / Batal`
...
- **Waktu respons UI daftar tender:** P95 kurang dari 1 detik
...
| **KP-T-004** | Tender dengan skor ≥70 menghasilkan notifikasi yang diterima pengguna dalam waktu kurang dari 60 detik | [ ]    |
...
| **RF-AI-007** | Proses generasi berjalan asinkron dengan notifikasi saat selesai (kurang dari 60 detik)   | Tinggi    |
...
- Generasi proposal selesai dalam waktu kurang dari 60 detik untuk dokumen 10 halaman
...
Internal Workspace adalah platform terpusat yang menjadi ruang kerja digital sehari-hari seluruh staf internal Cliste. App 2 dirancang dengan filosofi **satu tempat untuk semua**: tidak ada lagi informasi yang tersebar di WhatsApp, email, Google Drive, atau berbagai spreadsheet. Setiap task, dokumen, dan komunikasi dengan klien memiliki tempatnya yang pasti di dalam sistem ini.

App 2 terdiri dari **tiga modul** dalam satu platform:

1. **Project Management** sebagai inti
2. **Document Management** sebagai modul penyimpanan dokumen terstruktur
3. **Client Portal** sebagai antarmuka transparansi untuk klien eksternal
...
PM dapat membuat struktur proyek lengkap, mendistribusikan pekerjaan ke anggota tim, dan memantau progres secara real-time. Antarmuka utama adalah papan Kanban yang mendukung perbaruan status dengan drag-and-drop.
...
- Tambah anggota tim ke proyek dengan peran dalam proyek: **PM, Lead, Anggota, Reviewer, atau Observer**
...
Masalah utama yang ingin diselesaikan adalah **overload yang tidak terdeteksi**. Fitur ini memberikan visibilitas beban kerja proaktif sebelum penugasan dilakukan, bukan setelah engineer sudah kelelahan.
...
- **Indikator utilisasi berwarna:**
  ...
  - 🟡 **Kuning** — 80–95%
  - 🔴 **Merah** — di atas 95%
- Peringatan otomatis saat PM akan menugaskan task ke anggota dengan utilisasi lebih dari 90%
- Tampilan agregat per divisi untuk manajemen tingkat atas (tanpa menampilkan detail individu)

#### 3.3.3 Permintaan Lintas Divisi

- Formulir permintaan: deskripsi kebutuhan, divisi yang dituju, deadline kebutuhan, dan tingkat urgensi
...
Setiap dokumen dalam sistem memiliki identitas jelas: terhubung ke proyek atau tender, memiliki tipe sesuai taksonomi yang ditetapkan, dan setiap perubahan menghasilkan versi baru tanpa menghapus versi sebelumnya.
...
- Setiap akses unduhan tercatat di audit log untuk keperluan keamanan

#### 3.4.3 Pencarian Dokumen

- Pencarian berdasarkan nama file, tipe dokumen, fase proyek, dan tanggal unggah
- Pencarian full-text berdasarkan isi dokumen untuk PDF dan DOCX yang sudah diindeks (setelah proses ekstraksi teks selesai)
...
Klien mendapatkan akses ke informasi progres yang telah dikurasi oleh PM. Bukan seluruh detail internal, melainkan informasi yang relevan dan sudah disetujui untuk ditampilkan ke luar.
...
### 3.7 User Stories — App 2 Phase 1 (Pilihan Utama)
...
- **Waktu muat halaman awal (LCP):** kurang dari 2 detik pada koneksi 10 Mbps
- **Papan Kanban dengan 50 kartu task:** render di bawah 500ms
...
- **Notifikasi real-time:** pesan baru diterima pengguna dalam kurang dari 2 detik tanpa refresh halaman
...
> Menambahkan lapisan analitik, peningkatan notifikasi, dan fitur yang meningkatkan produktivitas tim setelah semua modul Phase 1 berjalan stabil.
...
Phase 3 memanfaatkan dokumen yang terkumpul selama operasional Phase 1 sebagai sumber pengetahuan. Teknologi **Retrieval-Augmented Generation (RAG)** dikombinasikan dengan penyimpanan vektor di `pgvector` memungkinkan engineer mengajukan pertanyaan kontekstual seperti:

> _"Bagaimana cara kita menangani masalah latensi sensor di proyek sebelumnya?"_

...dan mendapatkan jawaban relevan dari arsip dokumen teknis yang ada.
...
Satu-satunya titik integrasi antara App 1 dan App 2 terjadi saat tender dinyatakan berhasil dimenangkan. Proses ini bersifat **satu arah**, dari App 1 ke App 2.

#### Alur Proses

1. PM memperbarui status tender di App 1 menjadi **Menang**
2. Sistem menampilkan formulir **Buat Proyek** yang sudah terisi sebagian dengan data dari tender
3. PM melengkapi informasi: nama proyek (dapat diedit), PM penanggung jawab, tanggal mulai/selesai
4. Sistem membuat record proyek baru di App 2 dengan referensi ke ID tender asal
5. PM dan anggota tim mendapat notifikasi bahwa proyek baru telah dibuat
...
> Urutan berikut dirancang untuk memaksimalkan nilai yang dapat diuji di setiap tahap dan meminimalkan risiko rework akibat ketergantungan yang belum terpenuhi.
